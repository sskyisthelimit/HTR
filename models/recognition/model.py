...
class DTrOCRModel(pl.LightningModule):
    ...
    def forward(
        ...
        device = input_ids.device \
            if input_ids is not None else input_ids.device
...
class DTrOCRLMHeadModel(pl.LightningModule):
    def __init__(self, config: DTrOCRConfig):
        ...
        self.image_embedding_length = int((image_size[0] / patch_size[0]) *
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        has_eos_stopping_criteria = any(
            hasattr(criteria,
                    "eos_token_id") for criteria in stopping_criteria
                    )
        ...
        model_kwargs = self._get_initial_cache_position(input_ids,
                                                        model_kwargs)
        ...
        while not this_peer_finished:
            ...
            # Clone is needed to avoid keeping a hanging ref
            # to outputs.logits which may be very large for first
            ...
            # finished sentences should have their next token
            # be a padding token
            ...
            # Otherwise a reference to outputs is kept
            # which keeps the logits alive in the next iteration
            del outputs
    ...
    def _beam_search(
        ...
        **model_kwargs,
        ...
        # This makes sure that only tokens
        # of the first beam are considered to avoid sampling
        # the exact same tokens across all beams.
        ...
        while not this_peer_finished:
            ...
            # which may be very large for first
            # iteration (the clone itself is always small)
            next_token_logits = outputs.logits[:, -1, :].clone()
            ...
            next_token_scores = next_token_scores_processed + \
                beam_scores[:, None].expand_as(next_token_scores_processed)
            ...
            # Beam token selection: pick 1 + eos_token_id.shape[0] next tokens
            # for each beam so we have at least 1
            # non eos token per beam.
            n_tokens_to_keep = max(2, 1 + 1) * num_beams
            next_token_scores, next_tokens = torch.topk(
                next_token_scores, n_tokens_to_keep, dim=1,
            ...
            # This is needed to properly delete outputs.logits which
            # may be very large for first iteration
            # Otherwise a reference to outputs is kept which keeps the logits
            # alive in the next iteration
            # IMPORTANT: Note that this should appear BEFORE
            # the call to _reorder_cache() to save the maximum memory
    ...
    def _get_stopping_criteria(
        ...
        if generation_config.stop_strings is not None:
            if processor is None:
                raise ValueError(
                    "There are one or more stop strings, either in the "
                    "arguments to `generate` or in the "
                    "model's generation config, but we could not "
                    "locate a tokenizer. When generating with "
                    "stop strings, you must pass the model's tokenizer"
                    " to the `tokenizer` argument of `generate`."
                )
            criteria.append(StopStringCriteria(
                stop_strings=generation_config.stop_strings,
                tokenizer=processor.tokeniser)
            )
    ...
    def _reorder_cache(
        ...
        """
        This function is used to re-order the `past_key_values`
        cache if [`~PreTrainedModel.beam_search`] or
        [`~PreTrainedModel.beam_sample`] is called.
        This is required to match `past_key_values` with the correct
        beam_idx at every generation step.
        """
        return tuple(
            tuple(past_state.index_select(
                0, beam_idx.to(past_state.device)) for past_state in layer_past)
